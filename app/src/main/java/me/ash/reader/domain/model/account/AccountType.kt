...
class AccountType(val id: Int) {
     ...
     */
    init {
        if (id < 1 || id > 7) { // Updated range to include NextcloudNews
    ...
    fun toDesc(context: Context): String =
        when (this.id) {
            ...
            7 -> context.getString(R.string.nextcloud_news) // Added Nextcloud News
    ...
    fun toIcon(): Any =
        when (this.id) {
            ...
            7 -> painterResource(id = R.drawable.ic_nextcloud) // Added Nextcloud News icon (assuming ic_nextcloud exists or will be added)
    ...
    companion object {
        ...
        val NextcloudNews = AccountType(7) // Added Nextcloud News
    }
}
